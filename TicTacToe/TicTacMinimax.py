...
def evaluate_game(board):
    ...
    Win-states contains a list of tuples of size 3 that contain the board state of
...
def O_move(board):
    ...
    Just returns first avaiable space.
    Arguments:
    - board: the state of the current board
    Return:
...
def minimax(board, depth, isMax):
    ...
    if (result != 0):
        score = scores[result] #Where I incorporate the depth - score part to get an optimally runing solution
        return score
...
def X_move(board):
    ...
    #      Given an input game state, I try and find the best move for X with the minimax algorithm
    #      For scores, +10 is used for an X win, -10 for a O win, and 0 for a Draw
    #      In addition, in order to motivate the agent to win or lose as soon as possible, I
